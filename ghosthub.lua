...
function UpdateRealFruitChams()
    ...
    for _, v102 in pairs(game.Workspace.Ba TVSpawner:GetChildren()) do
...
local v466 = loadstring(game:HttpGet("https://pastefy.app/FnXvHR3a/raw"))():MakeWindow({
    Title = "isea hub",
    SubTitle = "by kchi roblox",
...
l_ImageButton_0.Image = "rbxassetid://116147971499582"
...
local v484 = v466:MakeTab({"Th\195\180ng Tin | Info", "rbxassetid://116147971499582"})
local v485 = v466:MakeTab({"Farming", "rbxassetid://116147971499582"})
local v486 = v466:MakeTab({"Auto Fishing", "rbxassetid://116147971499582"})
local v487 = v466:MakeTab({"Quest | Items", "rbxassetid://116147971499582"})
local v488 = v466:MakeTab({"Volcano Dojo", "rbxassetid://116147971499582"})
local v489 = v466:MakeTab({"Sea Event", "rbxassetid://116147971499582"})
local v490 = v466:MakeTab({"Race V4", "rbxassetid://116147971499582"})
local v491 = v466:MakeTab({"Raid | dungeon", "rbxassetid://116147971499582"})
local v492 = v466:MakeTab({"Fruits | Check Stock", "rbxassetid://116147971499582"})
local v493 = v466:MakeTab({"Teleport", "rbxassetid://116147971499582"})
local v494 = v466:MakeTab({"PvP,Player", "rbxassetid://116147971499582"})
local v495 = v466:MakeTab({"Shop", "rbxassetid://116147971499582"})
local v496 = v466:MakeTab({"Settings", "rbxassetid://116147971499582"})
v484:AddDiscordInvite({
    Name = "Tham Gia Discord",
    Description = "Link Sever Discord",
    Logo = "rbxassetid://116147971499582",
    Invite = "https://discord.gg/xY3hUExFEZ"
})
v484:AddDiscordInvite({
    Name = "isea hub",
...
function CheckQuestNew()
    local l = game.Players.LocalPlayer.Data.Level.Value

    if l >= 2600 and l <= 2624 then
        MonNew = "Reef Bandit"
        LevelQuestNew = 1
        NameQuestNew = "SubmergedQuest1"
        NameMonNew = "Reef Bandit"
        CFrameQuestNew = CFrame.new(10780.739, -2088.410, 9260.411)
        CFrameMonNew = CFrame.new(10899.859, -2145.235, 9279.294)

    elseif l >= 2625 and l <= 2649 then
        MonNew = "Coral Pirate"
        LevelQuestNew = 2
        NameQuestNew = "SubmergedQuest1"
    ...
    elseif l >= 2650 and l <= 2674 then
        MonNew = "Sea Chanter"
        LevelQuestNew = 1
        NameQuestNew = "SubmergedQuest2"
        NameMonNew = "Sea Chanter"
        CFrameQuestNew = CFrame.new(10883.599, -2086.885, 10034.020)
        CFrameMonNew = CFrame.new(10786.387, -2087.373, 10105.262)

    elseif l >= 2675 and l <= 2750 then
        ...
        if _G.AutoFarmLevelNew then
            pcall(function()
                ...
                if not QuestUI.Visible then
                    ...
                    local EnemyList = game.Workspace.Enemies:GetChildren()
                    local Found = false

                    for _, mob in pairs(EnemyList) do
                        if mob.Name == MonNew and mob:FindFirstChild("HumanoidRootPart") and mob.Humanoid.Health > 0 then
                            Found = true
                            ...
                            repeat task.wait()
                                ...
                                -- **CHỈ BAY LÊN ĐẦU MOB – KHÔNG BRING**
                            ...
                            until mob.Humanoid.Health <= 0 or not _G.AutoFarmLevelNew or QuestUI.Visible == false
                        end
                    end

                    -- Không có mob → teleport tới chỗ farm
                    if not Found then
                        TP1(CFrameMonNew)
                    end
                end
        ...
        if _G.AutoNear then
            pcall(function()
                for _, v522 in pairs(game.Workspace.Enemies:GetChildren()) do
                    if v522:FindFirstChild("Humanoid")
                    and v522:FindFirstChild("HumanoidRootPart")
                    and v522.Humanoid.Health > 0
                    and not string.find(v522.Name, "Shadow")
...
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            ...
            v589:Set("bạn có : " .. tostring(v590) .. " Bones")
        end)
    end
end)
v485:AddToggle({
...
local _ = v491:AddSection({"Dungeon"})
v491:AddToggle({
    Name = "Bring Mob (Dungeon)",
    Description = "Gom tất cả mob khi farm dungeon (tắt = Auto Dungeon không bring)",
    Default = true,
    Callback = function(v)
        _G.DungeonBring = v
        if not v then
...
_G.DungeonBring = _G.DungeonBring ~= false
...
local SOFT_BRING = true 
local SOFT_BRING_RADIUS = 250
local SOFT_BRING_PULL_Y = 4
local SOFT_BRING_STEP = 0.15
...
-- ====== TARGET PICK (ưu tiên trên trời) ======
local PRIORITY_NAME = "PropHitboxPlaceholder"
local PRIORITY_BONUS = 1000000

local SHADOW_NAME = "Blank Buddy" -- cái này KHÔNG đánh

local function IsValidMob(mob, hrp)
    if not mob or not mob.Parent then return false end

    -- ✅ bỏ qua Blank Buddy (shadow)
    if mob.Name == SHADOW_NAME then return false end
...
local function GetBestMob()
    ...
    for _, mob in pairs(workspace.Enemies:GetChildren()) do
        if IsValidMob(mob, hrp) then
            ...
            -- ✅ ưu tiên nhất PropHitboxPlaceholder
...
-- ====== FREEZE TAG (giữ như bạn) ======
local function FreezeMobOnce(mob)
    if mob:FindFirstChild("Frozen") then return end
...
-- ====== SOFT BRING (backup nếu StartBring không kéo) ======
...
-- tính vị trí trung bình theo "từng loại mob", nhưng chỉ tính những con đang trong vùng bring (đỡ ghost)
local function GetMobPositionByNameInRange(EnemiesName, centerPos, range)
    local pos = Vector3.zero
    local count = 0

    for _, v in pairs(workspace.Enemies:GetChildren()) do
        if v.Name == EnemiesName and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") then
            if v.Humanoid.Health > 0 then
                local p = v.HumanoidRootPart.Position
                if (p - centerPos).Magnitude <= range then
                    pos += p
                    count += 1
                end
            end
...
-- Bring mob theo logic file bạn gửi (Tween gom về vị trí trung bình theo từng name)
local function BringMobDungeon(enable)
    if not enable then return end
    if not _G.Dungeonh or _G.GoingExit or _G.DeathPause then return end
    if not IsDungeonWorld() or not IsInDungeon() then return end

    local char = LocalPlayer.Character
    local myHrp = char and char:FindFirstChild("HumanoidRootPart")
    if not myHrp then return end

    local RANGE = 250      -- giống file bạn (vùng mob quanh player)
    local PULL_DIST = 600  -- FIX: bỏ <=280 (kéo được nhiều con hơn, ít "2 con")
    local MIN_DIST = 5
    ...
    -- vị trí trung bình theo từng loại mob (chỉ tính mob trong vùng range)
    local totalpos = {}
    for _, v in pairs(enemies) do
        if v and v.Parent and v:FindFirstChild("HumanoidRootPart") then
            if not totalpos[v.Name] then
                totalpos[v.Name] = GetMobPositionByNameInRange(v.Name, myHrp.Position, RANGE)
            end
        end
    end
    ...
    -- gom mob lại
    for _, v in pairs(enemies) do
        if v and v.Parent and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") then
            local hrp = v.HumanoidRootPart
            local humanoid = v.Humanoid

            if humanoid.Health > 0 and not string.find(v.Name, "Shadow") then
                if (hrp.Position - myHrp.Position).Magnitude <= RANGE then
                    local mobPos = totalpos[v.Name]
                    if mobPos then
                        local targetCF = CFrame.new(mobPos.X, mobPos.Y, mobPos.Z)
                        local dist = (hrp.Position - targetCF.Position).Magnitude

                        if dist > MIN_DIST and dist <= PULL_DIST then
                            hrp.CanCollide = false
                            hrp.AssemblyLinearVelocity = Vector3.zero
                            hrp.AssemblyAngularVelocity = Vector3.zero

                            -- giống file bạn: xoá animator để mob đứng im (giảm văng/ghost)
                            pcall(function()
                                local anim = humanoid:FindFirstChildOfClass("Animator")
                                if anim then anim:Destroy() end
                            end)

                            TweenObject(hrp, targetCF, 300)
                        end
                    end
                end
            end
...
-- ====== FARM LOOP (1 mục tiêu tốt nhất, ưu tiên trên cao) ======
task.spawn(function()
    while task.wait(0.05) do
        ...
        pcall(function()
            ...
            if not IsValidMob(mob, hrp) then return end
...
StartBring = _G.DungeonBring and true or false
AutoHaki()
EquipWeapon(_G.SelectWeapon)

local mhrp = mob.HumanoidRootPart
local targetCF = mhrp.CFrame * CFrame.new(0, TARGET_OFFSET_Y, 0)
                ...
                -- bring backup (kéo thêm những con cùng tên / gần)
                BringMobDungeon(_G.DungeonBring)
